...
const LOCALHOST_REWRITE = (process.env.LOCALHOST_REWRITE || 'host.docker.internal').split(';')
...
app.get('/cluster/*', async (req, res) => {
  try {
    ...
    //
    // Users of this service sign the datasource url with the secret key so that only people with the key are able to
    // authorize datasources but we don't have to faff with persisting credentials.
    if (SECRET_KEY) {
      const verify = createHmac('sha256', SECRET_KEY)
      verify.write(source)
      
      if (verify.update(source).digest('base64') !== req.headers.authorization) {
